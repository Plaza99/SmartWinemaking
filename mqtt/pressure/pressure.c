...
PROCESS_NAME(process_for_pressure_sensor);
AUTOSTART_PROCESSES(&process_for_pressure_sensor);
...
PROCESS(process_for_temperature_sensor, "Pressure sensor process");
...
//PRESA DA TEMPERATURE, RIMUOVERE (?)
...
PROCESS_THREAD(process_for_temperature_sensor, ev, data) {
	...
	while(true) {
		...
		case STATE_SUBSCRIBED:	
			...
			LOG_INFO("New value of pressure: %d\n", temperature);
			
			sprintf(app_buffer, "{\"node\": %d, \"pressure\": %d}", node_id, temperature);
